...
class _ELdGraphState extends State<ELdGraph> {
  ...
  Future<Map<int, String>> calculateDutyDurations() async {
    Map<int, Duration> dutyDurations = {
      0: Duration.zero, // Off Duty
      1: Duration.zero, // Sleeper Berth
      2: Duration.zero, //Driving
      3: Duration.zero, // On Duty
    };
    ...
    for (final log in widget.dataPoints) {
      ...
      if (end.isBefore(start)) {
        ...
        DateTime nextDayStart = DateTime(end.year, end.month, end.day, 0, 0);
        dutyDurations[log.eventType!] =
            dutyDurations[log.eventType]! + midnightEnd.difference(start);
        dutyDurations[log.eventType!] =
            dutyDurations[log.eventType]! + end.difference(nextDayStart);
      } else {
        dutyDurations[log.eventType!] =
            dutyDurations[log.eventType]! + end.difference(start);
      }
    }
...
class StepLineGraphPainter extends CustomPainter {
  ...
  void _drawStepGraph(
    ...
    final Map<int, double> yPositions = {
      0: size.height - (yStep * 3.5), // Off Duty
      1: size.height - (yStep * 2.5), //  Sleeper Berth (SB)
      2: size.height - (yStep * 1.5), // Drive
      3: size.height - (yStep * 0.5), //On Duty
    };
    ...
    for (int i = 0; i < dataPoints.length; i++) {
      ...
      double startX = (timeToX(
        EldUtils.formatTo24Hour(log.startTime ?? _timeNow),
        size.width,
      ));
      double endX = (timeToX(
        EldUtils.formatTo24Hour(log.startTime ?? _timeNow),
      ...
      double y = yPositions[log.eventType]!; // Get Y position from map

      path.moveTo(startX, y);
      path.lineTo(endX, y);

      // Draw vertical transition line to next event type (only if not last log)
      if (i < dataPoints.length - 1) {
        double nextY = yPositions[dataPoints[i + 1].eventType]!;
